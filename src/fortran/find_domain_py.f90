...
module find_domain_py
      ...
      INTEGER(KIND=4) n_ptcl, n_mpi, n_thread, n_dim, levmax
contains
...
!Local Variables
      ...
      INTEGER(KIND=4) cpu_min(8), cpu_max(8)
      ...
      ALLOCATE(cpu_list(1:n_mpi))
      ALLOCATE(cpu_read(1:n_mpi))
      dom_list = -1

      call omp_set_num_threads(n_thread)

      DO i=1, n_ptcl
        xmin = MAX(xc(i) - rr(i), 0.0)
        xmax = MIN(xc(i) + rr(i), 1.0)
        ymin = MAX(yc(i) - rr(i), 0.0)
        ymax = MIN(yc(i) + rr(i), 1.0)
        zmin = MAX(zc(i) - rr(i), 0.0)
        zmax = MIN(zc(i) + rr(i), 1.0)
