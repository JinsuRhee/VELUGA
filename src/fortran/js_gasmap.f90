!234567
      SUBROUTINE js_gasmap(larr, darr, xx, yy, zz, bw, xr, yr, map)
      ...
      REAL(KIND=8) xx(larr(1)), yy(larr(1)), zz(larr(1),2)
      REAL(KIND=8) bw(2), xr(2), yr(2)
      REAL(KIND=8) map(larr(2), larr(2), 2)
      ...
      DO i=1, n_cell
        ...
        DO k=ny0, ny1
          ...
          ELSE IF (amrtype .EQ. 4) THEN !! Column Density
                  map(j,k,1)    = map(j,k,1) + zz(i,2)*geometry*bw(1)
